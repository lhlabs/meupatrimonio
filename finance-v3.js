...
async function col(db,uid,name){
  const s=await getDocs(collection(db,'users',uid,name));
  return s.docs.map(d=>({id:d.id,...d.data()}));
}
function isContribution(t){
  const c=norm(t?.category);
  return t?.type==='expense'&&(c.includes('aporte')||c.includes('investimento'));
}
function monthRows(tx,d=new Date()){
  const k=monthKey(d);
  return tx.filter(t=>String(t.date||'').startsWith(k));
}
...
function activeRecurringExpense(r,today){
  ...
  return Number(r.amount||0)>0&&!isContribution(r);
}

function addContributionOption(select){
  if(!select)return;
  if(![...select.options].some(o=>o.value===contributionCategory))select.add(new Option(contributionCategory,contributionCategory));
}
function injectContributionCategory(){
  if(document.querySelector('#transactionType')?.value==='expense')addContributionOption(document.querySelector('#transactionCategory'));
  if(document.querySelector('#recurringType')?.value==='expense')addContributionOption(document.querySelector('#recurringCategory'));
  if(document.querySelector('#scheduledType')?.value==='expense')addContributionOption(document.querySelector('#scheduledCategory'));
}
...
function relabelGoalUI(){
  ...
  if(form){
    form.querySelectorAll('label').forEach(l=>{
      ...
      if(inp?.id==='monthlyGoalSurplus'&&l.firstChild?.nodeType===3)l.firstChild.textContent='Meta de aporte do mês';
      if(inp?.id==='monthlyGoalDaily'&&l.firstChild?.nodeType===3)l.firstChild.textContent='Limite de gastos do mês';
    });
  ...
  if(goalCard){
    ...
    const spans=goalCard.querySelectorAll('.goal-grid > div > span');
...
async function refresh(db,user){
  ...
  const contributionGoal=Number(goal?.monthlySurplusGoal||0);
  const spendGoal=Number(goal?.dailySpendGoal||0);
  ...
  if(income>0){
    setText('#savingRate',`${(contributions/income*100).toFixed(1)}%`);
    setText('#savingStatus',`${currency.format(contributions)} aportados de ${currency.format(income)} recebidos`);
  }else{
    setText('#savingRate','—');
    setText('#savingStatus','Sem receita lançada no mês');
  }
  ...
  if(health!==null){
    if(health>=85){avatar='🐷✨';state='Radiante';msg='Aportes, gastos e reserva estão muito bem alinhados.';}
    else if(health>=70){state='Saudável';msg='Boa disciplina financeira neste mês.';}
  ...
  setText('#scoreHint',health===null?'Sem nota até existirem indicadores calculáveis.':'Score: aportes 40% + gastos 35% + reserva 25%, considerando apenas indicadores disponíveis.');

  const missions=document.querySelector('#missionsList');
  if(missions){
    const items=[
      ['📈','Meta de aportes',contributionGoal>0?`${currency.format(contributions)} / ${currency.format(contributionGoal)}`:'Defina uma meta mensal',contributionGoal>0?clamp(contributions/contributionGoal,0,1):0],
      ['🎯','Limite de gastos',spendGoal>0?`${currency.format(consumption)} / ${currency.format(spendGoal)}`:'Defina um limite mensal',spendGoal>0?(consumption<=spendGoal?1:clamp(spendGoal/Math.max(consumption,.01),0,1)):0],
      ['🛟','Reserva de emergência',recurringMonthly>0?`${currency.format(reserve)} / ${currency.format(reserveTarget)}`:'Cadastre despesas recorrentes ativas',reserveProgress??0]
    ];
...
window.addEventListener('load',()=>{
  ...
  document.addEventListener('click',e=>{
    if(e.target.closest('#quickAddBtn,#openTransactionBtn,#openRecurringBtn,#openScheduledBtn,[data-delete-tx],[data-delete-position],[data-del-rec],[data-edit-rec],#prevMonth,#nextMonth,[data-go]')){
      setTimeout(()=>{relabelGoalUI();injectContributionCategory();run();},120);
    }
  });
